      ...
      <el-col :span="16">
        <div class="forecast-option">
          ...
          <div>
            预测数据来源：
            <el-select
                v-model="forecastDataSourceValue"
                placeholder="预测数据来源"
                @change="getStockDaily(forecastDaysCount, newsTypeRadio)"
                style="width: 170px;"
            >
              <el-option
                  v-for="item in forecastDataSource"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
              />
...
//预测数据来源
const forecastDataSource = [
  {'value': 1, 'label': '基础情感分析'},
  {'value': 2, 'label': '情感分数分析(Beta)'},
]
...
const getLastDaysScore = (forecastDaysCount: number, newsType: string) => {
  let partUrl = ''
  if (1 === forecastDataSourceValue.value) {
    partUrl = 'getLastDaysDailyNewsEmotionScoreList'
  } else if (2 === forecastDataSourceValue.value) {
    //使用语言模型分析出的分数来打出股票情绪分数
    partUrl = 'getLastDaysDailyNewsEmotionScoreListV2'
  }
  //在计算均线前，先获取最新一日的新闻情绪分数
  axios.get(`${baseUrls.crawler}/${partUrl}?stock_code=${selectStock.value}&days_count=${forecastDaysCount}&news_type=${newsType}`)
      .then((response) => {
